...
describe('acknowledgeRequest — valid ack', () => {
    it('returns ok:true with r2pId and status=delivered', () => {
        ...
        forceStatus(id, 'sent');
        const result = (0, r2pRequest_1.acknowledgeRequest)(id, ackInput);
    ...
    it('updates status to delivered in the store', () => {
        ...
        expect(r2pRequest_1.r2pRepo.findById(id)?.status).toBe('delivered');
    });
...
describe('acknowledgeRequest — persistence', () => {
    it('persists acknowledgement row with correct fields', () => {
        ...
        forceStatus(id, 'sent');
        (0, r2pRequest_1.acknowledgeRequest)(id, ackInput);
        const ack = r2pRequest_1.ackRepo.findByR2PId(id);
...
describe('acknowledgeRequest — ALREADY_ACKNOWLEDGED', () => {
    it('returns ALREADY_ACKNOWLEDGED on second call for same r2pId', () => {
        ...
        forceStatus(id, 'sent');
        (0, r2pRequest_1.acknowledgeRequest)(id, ackInput);
        const result = (0, r2pRequest_1.acknowledgeRequest)(id, { participantId: 'BANK_B', receivedAt: '2026-07-01T11:00:00Z' });
    ...
    it('does not overwrite existing ack row on duplicate', () => {
        ...
        forceStatus(id, 'sent');
        (0, r2pRequest_1.acknowledgeRequest)(id, ackInput);
        (0, r2pRequest_1.acknowledgeRequest)(id, { participantId: 'BANK_B', receivedAt: '2026-07-01T11:00:00Z' });
        const ack = r2pRequest_1.ackRepo.findByR2PId(id);
        expect(ack?.participant_id).toBe(ackInput.participantId);
    });
});
...
describe('acknowledgeRequest — NOT_FOUND', () => {
    it('returns NOT_FOUND for a non-existent r2pId', () => {
        const result = (0, r2pRequest_1.acknowledgeRequest)('00000000-0000-7000-0000-000000000000', ackInput);
        expect(result.ok).toBe(false);
        if (result.ok)
            return;
...
describe('acknowledgeRequest — event emission', () => {
    it('appends REQUEST_ACKNOWLEDGED audit entry', () => {
        ...
        forceStatus(id, 'sent');
        (0, r2pRequest_1.acknowledgeRequest)(id, ackInput);
        const acks = r2pRequest_1.auditRepo.list().filter((a) => a.event_type === 'REQUEST_ACKNOWLEDGED');
    ...
    it('appends EVENT_ACKNOWLEDGED_EMITTED audit entry', () => {
        ...
        forceStatus(id, 'sent');
        (0, r2pRequest_1.acknowledgeRequest)(id, ackInput);
        const emitted = r2pRequest_1.auditRepo.list().filter((a) => a.event_type === 'EVENT_ACKNOWLEDGED_EMITTED');
        expect(emitted).toHaveLength(1);
        expect(emitted[0].r2p_id).toBe(id);
        expect(emitted[0].actor).toBe('event-publisher');
    });
    it('does not emit event on NOT_FOUND', () => {
        (0, r2pRequest_1.acknowledgeRequest)('nonexistent-id', ackInput);
        const emitted = r2pRequest_1.auditRepo.list().filter((a) => a.event_type === 'EVENT_ACKNOWLEDGED_EMITTED');
...
describe('acknowledgeRequest — state transition', () => {
    it('appends transition from sent → delivered', () => {
        ...
        forceStatus(id, 'sent');
        (0, r2pRequest_1.acknowledgeRequest)(id, ackInput);
        const transitions = r2pRequest_1.transitionRepo.list().filter((t) => t.r2p_id === id && t.to_status === 'delivered');
    ...
    it('does not append transition on failure', () => {
        (0, r2pRequest_1.acknowledgeRequest)('nonexistent-id', ackInput);
        const transitions = r2pRequest_1.transitionRepo.list().filter((t) => t.to_status === 'delivered');
...
describe('resetStore() — isolation', () => {
    it('clears acknowledgements between tests', () => {
        ...
        const result = (0, r2pRequest_1.acknowledgeRequest)(id2, ackInput);
        expect(result.ok).toBe(true);
    });
});
//# sourceMappingURL=acknowledgeRequest.test.js.map