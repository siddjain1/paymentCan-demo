...
describe('cancelRequest — valid cancel', () => {
    it('returns ok:true with r2pId, status=cancelled, cancelledAt for created state', () => {
        ...
        const result = (0, r2pRequest_1.cancelRequest)(id);
...
describe('cancelRequest — persistence', () => {
    it('status is updated to cancelled in the store', () => {
        const id = seedRequest();
        (0, r2pRequest_1.cancelRequest)(id);
        const row = r2pRequest_1.r2pRepo.findById(id);
...
describe('cancelRequest — NOT_FOUND', () => {
    it('returns NOT_FOUND for a non-existent r2pId', () => {
        const result = (0, r2pRequest_1.cancelRequest)('00000000-0000-7000-0000-000000000000');
...
describe('cancelRequest — audit trail', () => {
    it('appends REQUEST_CANCELLED audit entry after successful cancel', () => {
        const id = seedRequest();
        (0, r2pRequest_1.cancelRequest)(id);
        const audits = r2pRequest_1.auditRepo.list().filter((a) => a.event_type === 'REQUEST_CANCELLED');
    ...
    it('does not append audit entry on NOT_FOUND', () => {
        (0, r2pRequest_1.cancelRequest)('nonexistent-id');
        const cancelled = r2pRequest_1.auditRepo.list().filter((a) => a.event_type === 'REQUEST_CANCELLED');
...
describe('cancelRequest — state transition', () => {
    it('appends transition from created → cancelled', () => {
        const id = seedRequest();
        (0, r2pRequest_1.cancelRequest)(id);
        const transitions = r2pRequest_1.transitionRepo.list().filter((t) => t.r2p_id === id && t.to_status === 'cancelled');
    ...
    it('does not append transition on failure', () => {
        (0, r2pRequest_1.cancelRequest)('nonexistent-id');
        const transitions = r2pRequest_1.transitionRepo.list().filter((t) => t.to_status === 'cancelled');
