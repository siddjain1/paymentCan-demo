...
import { extractUpBlock, extractDownBlock, runMigrations } from '../db/migrate'
import {
  TEST_PARTICIPANTS,
  PARTICIPANT_MAP,
  seedParticipants,
...
import type {
  R2PRequestRow,
  R2PStateTransitionRow,
  R2PAcknowledgementRow,
  R2PResponseRow,
  R2PPaymentRow,
...
describe('runMigrations — behaviour with mock PoolClient', () => {
  function buildMockClient(appliedFilenames: string[] = []) {
  ...
  test('rolls back and throws on migration failure', async () => {
    ...
    await expect(runMigrations(mockClient as any)).rejects.toThrow('Migration')
    const rollbackCalls = mockClient.query.mock.calls.filter(
      (c) => String(c[0]).trim() === 'ROLLBACK'
    )
...
describe('schema.ts — typed interfaces', () => {
  test('R2PStateTransitionRow has from_status as string | null', () => {
    const row: R2PStateTransitionRow = {
      id: 'uuid',
      request_id: 'req-uuid',
      from_status: null,
      to_status: 'sent',
  ...
  test('EventSubscriptionRow active is boolean', () => {
    const row: EventSubscriptionRow = {
      id: 'uuid',
      participant_id: 'BANK_A',
      proxy_type: 'email',
  ...
  test('R2PAcknowledgementRow has all required fields', () => {
    const row: R2PAcknowledgementRow = {
      id: 'uuid',
      request_id: 'req-uuid',
...
describe('seed.ts — test participants', () => {
  ...
  test('seedParticipants calls INSERT with ON CONFLICT DO NOTHING for each participant', async () => {
    ...
    await seedParticipants(mockClient as any)
