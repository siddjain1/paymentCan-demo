...
import { createRequest, resetStore, r2pRepo } from '../../services/r2pRequest'
...
let deleteHandlers: ((...args: unknown[]) => unknown)[] = []
...
beforeAll(() => {
  ...
  if (!deleteRoute) throw new Error('DELETE /r2p/requests/:r2pId route not registered')
...
function seedR2P(): string {
  const result = createRequest(seedBody)
...
describe('DELETE /r2p/requests/:r2pId — valid cancel', () => {
  it('returns 200 with r2pId, status=cancelled, cancelledAt', () => {
    ...
    const body = res.body as { r2pId: string; status: string; cancelledAt: string }
