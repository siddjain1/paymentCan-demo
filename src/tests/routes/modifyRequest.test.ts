...
import { createRequest, resetStore } from '../../services/r2pRequest'
...
let patchHandlers: ((...args: unknown[]) => unknown)[] = []
...
beforeAll(() => {
  ...
  if (!patchRoute) throw new Error('PATCH /r2p/requests/:r2pId route not registered')
...
describe('PATCH /r2p/requests/:r2pId — valid patch', () => {
  it('returns 200 with r2pId, status, updatedAt when amount is changed', () => {
    ...
    const body = res.body as { r2pId: string; status: string; updatedAt: string }
