...
describe('modifyRequest — valid patch', () => {
  it('returns ok:true with r2pId, status, updatedAt when amount is patched', () => {
    ...
    const result = modifyRequest(id, { amount: 200 })
...
describe('modifyRequest — persistence', () => {
  ...
  it('persists dueDate change', () => {
    const id = seedRequest()
    modifyRequest(id, { dueDate: '2026-09-15' })
    const row = r2pRepo.findById(id)
  ...
  it('persists expiryTimestamp change', () => {
    const id = seedRequest()
    modifyRequest(id, { expiryTimestamp: '2026-09-15T00:00:00+05:30' })
    const row = r2pRepo.findById(id)
  ...
  it('persists remittanceInfo change', () => {
    const id = seedRequest()
    modifyRequest(id, { remittanceInfo: 'New note' })
    const row = r2pRepo.findById(id)
...
describe('modifyRequest — NOT_FOUND', () => {
  it('returns NOT_FOUND for unknown r2pId', () => {
    const result = modifyRequest('00000000-0000-7000-0000-000000000000', { amount: 10 })
...
describe('modifyRequest — INVALID_STATE_TRANSITION', () => {
  ...
  terminalStates.forEach((state) => {
    it(`returns INVALID_STATE_TRANSITION when status is ${state}`, () => {
      ...
      const result = modifyRequest(id, { amount: 10 })
      expect(result.ok).toBe(false)
      if (result.ok) return
  ...
  it('allows modification when status is sent', () => {
    ...
    const result = modifyRequest(id, { amount: 75 })
    expect(result.ok).toBe(true)
  })
})
...
describe('modifyRequest — NO_FIELDS_TO_UPDATE', () => {
  ...
  it('returns NO_FIELDS_TO_UPDATE when only unrecognised keys present', () => {
    ...
    const result = modifyRequest(id, {} as { amount?: number })
    expect(result.ok).toBe(false)
    if (result.ok) return
    expect(result.code).toBe('NO_FIELDS_TO_UPDATE')
  })
...
describe('modifyRequest — VALIDATION_ERROR', () => {
  it('rejects negative amount', () => {
    const id = seedRequest()
    const result = modifyRequest(id, { amount: -1 })
    expect(result.ok).toBe(false)
    if (result.ok) return
  ...
  it('rejects zero amount', () => {
    const id = seedRequest()
    const result = modifyRequest(id, { amount: 0 })
    expect(result.ok).toBe(false)
    if (result.ok) return
    expect(result.code).toBe('VALIDATION_ERROR')
  })
  ...
  it('rejects non-date dueDate', () => {
    const id = seedRequest()
    const result = modifyRequest(id, { dueDate: 'not-a-date' })
    expect(result.ok).toBe(false)
    if (result.ok) return
    expect(result.code).toBe('VALIDATION_ERROR')
  })
  ...
  it('rejects dueDate without proper YYYY-MM-DD format', () => {
    const id = seedRequest()
    const result = modifyRequest(id, { dueDate: '01-07-2026' })
    expect(result.ok).toBe(false)
    if (result.ok) return
    expect(result.code).toBe('VALIDATION_ERROR')
  })
  ...
  it('rejects expiryTimestamp without timezone', () => {
    const id = seedRequest()
    const result = modifyRequest(id, { expiryTimestamp: '2026-07-01T23:59:59' })
    expect(result.ok).toBe(false)
    if (result.ok) return
    expect(result.code).toBe('VALIDATION_ERROR')
  })
  ...
  it('rejects invalid expiryTimestamp', () => {
    const id = seedRequest()
    const result = modifyRequest(id, { expiryTimestamp: 'not-a-timestamp' })
    expect(result.ok).toBe(false)
    if (result.ok) return
    expect(result.code).toBe('VALIDATION_ERROR')
  })
...
describe('modifyRequest — audit trail', () => {
  it('appends REQUEST_MODIFIED audit entry after successful patch', () => {
    const id = seedRequest()
    modifyRequest(id, { amount: 99 })

    const audits = auditRepo.list().filter((a) => a.event_type === 'REQUEST_MODIFIED')
  ...
  it('does not append audit entry on failure', () => {
    modifyRequest('nonexistent-id', { amount: 10 })
    const modified = auditRepo.list().filter((a) => a.event_type === 'REQUEST_MODIFIED')
...
describe('modifyRequest — state transition', () => {
  it('appends transition with from_status === to_status', () => {
    ...
    const transitions = transitionRepo.list().filter(
      (t) => t.r2p_id === id && t.from_status === 'created'
    )
