...
describe('responseValidation — amount matches', () => {
  it('succeeds when amount exactly matches request amount', () => {
    const id = seedDelivered()
    const result = respondToRequest(id, { responseType: 'accept', participantId: 'BANK_A', respondedAt: '2026-07-01T11:00:00Z', amount: REQUEST_AMOUNT })
    expect(result.ok).toBe(true)
    if (!result.ok) return
  ...
  it('amount match works for decline too', () => {
    const id = seedDelivered()
    const result = respondToRequest(id, { responseType: 'decline', participantId: 'BANK_A', respondedAt: '2026-07-01T11:00:00Z', amount: REQUEST_AMOUNT })
    expect(result.ok).toBe(true)
  })

  it('amount match works for defer too', () => {
    const id = seedDelivered()
    const result = respondToRequest(id, { responseType: 'defer', participantId: 'BANK_A', respondedAt: '2026-07-01T11:00:00Z', amount: REQUEST_AMOUNT })
    expect(result.ok).toBe(true)
  })
})
...
describe('responseValidation — AMOUNT_MISMATCH', () => {
  it('returns AMOUNT_MISMATCH when amount is higher than request amount', () => {
    const id = seedDelivered()
    const result = respondToRequest(id, { responseType: 'accept', participantId: 'BANK_A', respondedAt: '2026-07-01T11:00:00Z', amount: REQUEST_AMOUNT + 1 })
  ...
  it('returns AMOUNT_MISMATCH for decline with wrong amount', () => {
    const id = seedDelivered()
    const result = respondToRequest(id, { responseType: 'decline', participantId: 'BANK_A', respondedAt: '2026-07-01T11:00:00Z', amount: 999 })
    expect(result.ok).toBe(false)
    if (result.ok) return
    expect(result.code).toBe('AMOUNT_MISMATCH')
  })
...
describe('responseValidation — VALIDATION_ERROR on bad amount', () => {
  it('returns VALIDATION_ERROR for amount = 0', () => {
    const id = seedDelivered()
    const result = respondToRequest(id, { responseType: 'accept', participantId: 'BANK_A', respondedAt: '2026-07-01T11:00:00Z', amount: 0 })
    expect(result.ok).toBe(false)
    if (result.ok) return
  ...
  it('returns VALIDATION_ERROR for negative amount', () => {
    const id = seedDelivered()
    const result = respondToRequest(id, { responseType: 'accept', participantId: 'BANK_A', respondedAt: '2026-07-01T11:00:00Z', amount: -50 })
    expect(result.ok).toBe(false)
    if (result.ok) return
    expect(result.code).toBe('VALIDATION_ERROR')
  })
  ...
  it('returns VALIDATION_ERROR for Infinity', () => {
    const id = seedDelivered()
    const result = respondToRequest(id, { responseType: 'accept', participantId: 'BANK_A', respondedAt: '2026-07-01T11:00:00Z', amount: Infinity })
    expect(result.ok).toBe(false)
    if (result.ok) return
    expect(result.code).toBe('VALIDATION_ERROR')
  })
  ...
  it('returns VALIDATION_ERROR for NaN', () => {
    const id = seedDelivered()
    const result = respondToRequest(id, { responseType: 'accept', participantId: 'BANK_A', respondedAt: '2026-07-01T11:00:00Z', amount: NaN })
    expect(result.ok).toBe(false)
    if (result.ok) return
    expect(result.code).toBe('VALIDATION_ERROR')
  })
...
describe('responseValidation — guard ordering', () => {
  it('returns INVALID_STATE_TRANSITION before checking amount', () => {
    const id = seedDelivered()
    const row = r2pRepo.findById(id)!
    r2pRepo.update(id, { updated_at: new Date().toISOString(), status: 'accepted' }, row.version)
    const result = respondToRequest(id, { responseType: 'accept', participantId: 'BANK_A', respondedAt: '2026-07-01T11:00:00Z', amount: 999 })
    expect(result.ok).toBe(false)
    if (result.ok) return
